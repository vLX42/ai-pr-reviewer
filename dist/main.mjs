...
var SimpleMemory = class extends BaseMemory {
  constructor() {
    super();
...
var Bot = class {
  constructor(options, openaiOptions) {
    ...
    __publicField(this, "memory", null);
    __publicField(this, "options");
    __publicField(this, "chat", async (message) => {
      try {
    ...
    __publicField(this, "chat_", async (message) => {
...
// src/options.mts
import { info as info2 } from "@actions/core";
...
var TokenLimits = class {
  constructor(model = "gpt-4o") {
    ...
    __publicField(this, "requestTokens");
    __publicField(this, "responseTokens");
    __publicField(this, "knowledgeCutOff");
...
var Options = class {
  constructor(debug, disableReview, disableReleaseNotes, maxFiles = "0", reviewSimpleChanges = false, reviewCommentLGTM = false, pathFilters = null, systemMessage = "", openaiLightModel = "gpt-4o", openaiHeavyModel = "gpt-4o", openaiModelTemperature = "0.0", openaiRetries = "3", openaiTimeoutMS = "120000", openaiConcurrencyLimit = "6", githubConcurrencyLimit = "6", language = "en-US") {
    ...
    __publicField(this, "disableReview");
    __publicField(this, "disableReleaseNotes");
    __publicField(this, "maxFiles");
    __publicField(this, "reviewSimpleChanges");
    __publicField(this, "reviewCommentLGTM");
    __publicField(this, "pathFilters");
    __publicField(this, "systemMessage");
    __publicField(this, "openaiLightModel");
    __publicField(this, "openaiHeavyModel");
    __publicField(this, "openaiModelTemperature");
    __publicField(this, "openaiRetries");
    __publicField(this, "openaiTimeoutMS");
    __publicField(this, "openaiConcurrencyLimit");
    __publicField(this, "githubConcurrencyLimit");
    __publicField(this, "lightTokenLimits");
    __publicField(this, "heavyTokenLimits");
  ...
  print() {
    info2(`debug: ${this.debug}`);
    info2(`disable_review: ${this.disableReview}`);
    info2(`disable_release_notes: ${this.disableReleaseNotes}`);
    info2(`max_files: ${this.maxFiles}`);
    info2(`review_simple_changes: ${this.reviewSimpleChanges}`);
    info2(`review_comment_lgtm: ${this.reviewCommentLGTM}`);
    info2(`path_filters: ${this.pathFilters}`);
    info2(`system_message: ${this.systemMessage}`);
    info2(`openai_light_model: ${this.openaiLightModel}`);
    info2(`openai_heavy_model: ${this.openaiHeavyModel}`);
    info2(`openai_model_temperature: ${this.openaiModelTemperature}`);
    info2(`openai_retries: ${this.openaiRetries}`);
    info2(`openai_timeout_ms: ${this.openaiTimeoutMS}`);
    info2(`openai_concurrency_limit: ${this.openaiConcurrencyLimit}`);
    info2(`github_concurrency_limit: ${this.githubConcurrencyLimit}`);
    info2(`summary_token_limits: ${this.lightTokenLimits.string()}`);
    info2(`review_token_limits: ${this.heavyTokenLimits.string()}`);
    info2(`language: ${this.language}`);
  }
  checkPath(path) {
    const ok = this.pathFilters.check(path);
    info2(`checking path: ${path} => ${ok}`);
    return ok;
  }
};
...
var OpenAIOptions = class {
  constructor(model = "gpt-4o", tokenLimits = null) {
    ...
    __publicField(this, "tokenLimits");
    this.model = model;
    if (tokenLimits != null) {
...
var Prompts = class {
  constructor(summarize = "", summarizeReleaseNotes = "") {
    __publicField(this, "summarize");
    __publicField(this, "summarizeReleaseNotes");
    __publicField(this, "summarizeFileDiff", `## GitHub PR Title
...
`);
    __publicField(this, "triageFileDiff", `Below the summary, I would also like you to triage the diff as \`NEEDS_REVIEW\` or 
...
`);
    __publicField(this, "summarizeChangesets", `Provided below are changesets in this pull request. Changesets 
...
$raw_summary
`);
    __publicField(this, "summarizePrefix", `Here is the summary of changes you have generated for files:
      \`\`\`
      $raw_summary
      \`\`\`

`);
    __publicField(this, "summarizeShort", `Your task is to provide a concise summary of the changes. This 
...
`);
    __publicField(this, "reviewFileDiff", `## GitHub PR Title
...
$patches
`);
    __publicField(this, "comment", `A comment was made on a GitHub PR review for a 
...
// src/review.mts
import { error, info as info4, warning as warning4 } from "@actions/core";
import { context as github_context } from "@actions/github";
import pLimit from "p-limit";

// src/commenter.mts
import { getInput as getInput2, info as info3, warning as warning3 } from "@actions/core";
import { context } from "@actions/github";

// src/octokit.mts
import { getInput, warning as warning2 } from "@actions/core";
import { Octokit } from "@octokit/action";
import { retry } from "@octokit/plugin-retry";
import { throttling } from "@octokit/plugin-throttling";
var token = getInput("token") || process.env.GITHUB_TOKEN;
var RetryAndThrottlingOctokit = Octokit.plugin(throttling, retry);
...
`
      ...
      if (retryCount <= 3) {
        warning2(`Retrying after ${retryAfter} seconds!`);
        return true;
      }
    },
...
var COMMENT_GREETING = `${getInput2("bot_icon")}   CodeSailor`;
...
var Commenter = class {
  constructor() {
    __publicField(this, "reviewCommentsBuffer", []);
    __publicField(this, "reviewCommentsCache", {});
    __publicField(this, "issueCommentsCache", {});
  }
  /**
...
${tag}`;
    if (mode === "create") {
      ...
      warning3(`Unknown mode: ${mode}, use "replace" instead`);
      await this.replace(body, tag, target);
    }
  }
    ...
    try {
      ...
      if (pendingReview) {
        ...
        try {
          ...
          warning3(`Failed to delete pending review: ${e}`);
        }
      }
    } catch (e) {
      warning3(`Failed to list reviews: ${e}`);
    }
  }
...
`;
    if (this.reviewCommentsBuffer.length === 0) {
      info3(`Submitting empty review for PR #${pullNumber}`);
      try {
        ...
        warning3(`Failed to submit empty review: ${e}`);
      }
      return;
    }
      ...
      for (const c of comments) {
        if (c.body.includes(COMMENT_TAG)) {
          ...
          try {
            ...
            warning3(`Failed to delete review comment: ${e}`);
          }
        }
      }
    }
    ...
    try {
      ...
      for (const comment of this.reviewCommentsBuffer) {
        ...
        try {
          ...
          warning3(`Failed to create review comment: ${ee}`);
        }
        commentCounter++;
        info3(
...
`;
    try {
      ...
      warning3(`Failed to reply to the top-level comment ${error2}`);
      try {
        await octokit.pulls.createReplyForReviewComment({
          owner: repo.owner,
          repo: repo.repo,
        ...
        warning3(`Failed to reply to the top-level comment ${e}`);
      }
    }
    try {
      ...
      warning3(`Failed to update the top-level comment ${error2}`);
    }
  }
...
`;
  ...
  async getCommentChain(pullNumber, comment) {
    try {
      ...
      warning3(`Failed to get conversation chain: ${e}`);
      return {
        chain: "",
  ...
  async listReviewComments(target) {
    ...
    try {
      ...
      warning3(`Failed to list review comments: ${e}`);
      return allComments;
    }
  }
  async create(body, target) {
    try {
      ...
      warning3(`Failed to create comment: ${e}`);
    }
  }
  async replace(body, tag, target) {
    try {
      ...
      warning3(`Failed to replace comment: ${e}`);
    }
  }
  async findCommentWithTag(tag, target) {
    try {
      ...
      warning3(`Failed to find comment with tag: ${e}`);
      return null;
    }
  }
  async listComments(target) {
    ...
    try {
      ...
      warning3(`Failed to list comments: ${e}`);
      return allComments;
    }
  }
...
var Inputs = class _Inputs {
  constructor(systemMessage = "", title = "no title provided", description = "no description provided", rawSummary = "", shortSummary = "", filename = "", fileContent = "file contents cannot be provided", fileDiff = "file diff cannot be provided", patches = "", diff = "no diff", commentChain = "no other comments on this patch", comment = "no comment provided") {
    ...
    __publicField(this, "rawSummary");
    __publicField(this, "shortSummary");
    __publicField(this, "filename");
    __publicField(this, "fileContent");
    __publicField(this, "fileDiff");
    __publicField(this, "patches");
    __publicField(this, "diff");
    __publicField(this, "commentChain");
...
var codeReview = async (lightBot, heavyBot, options, prompts) => {
  ...
  if (context2.payload.pull_request == null) {
    warning4("Skipped: context.payload.pull_request is null");
    return;
  }
  ...
  if (inputs.description.includes(ignoreKeyword)) {
    info4("Skipped: description contains ignore_keyword");
    return;
  }
  ...
  if (highestReviewedCommitId === "" || highestReviewedCommitId === context2.payload.pull_request.head.sha) {
    ...
    info4(`Will review from commit: ${highestReviewedCommitId}`);
  }
  ...
  if (incrementalFiles == null || targetBranchFiles == null) {
    warning4("Skipped: files data is missing");
    return;
  }
  ...
  if (files.length === 0) {
    warning4("Skipped: files is null");
    return;
  }
  ...
  for (const file of files) {
    if (!options.checkPath(file.filename)) {
      info4(`skip for excluded path: ${file.filename}`);
  ...
  if (filterSelectedFiles.length === 0) {
    warning4("Skipped: filterSelectedFiles is null");
    return;
  }
  const commits = incrementalDiff.data.commits;
  if (commits.length === 0) {
    warning4("Skipped: commits is null");
    return;
  }
...
`;
  ...
  const doSummary = async (filename, fileContent, fileDiff) => {
    info4(`summarize: ${filename}`);
    const ins = inputs.clone();
    if (fileDiff.length === 0) {
      warning4(`summarize: file_diff is empty, skip ${filename}`);
    ...
    if (tokens > options.lightTokenLimits.requestTokens) {
      info4(`summarize: diff tokens exceeds limit, skip ${filename}`);
    ...
    try {
      ...
      if (summarizeResp === "") {
        info4("summarize: nothing obtained from openai");
        ...
        if (options.reviewSimpleChanges === false) {
          ...
          if (triageMatch != null) {
            ...
            info4(`filename: ${filename}, triage: ${triage}`);
      ...
      warning4(`summarize: error from openai: ${e}`);
...
`;
      ...
      if (summarizeResp === "") {
        warning4("summarize: nothing obtained from openai");
      } else {
  ...
  if (options.disableReleaseNotes === false) {
    ...
    if (releaseNotesResponse === "") {
      info4("release notes: nothing obtained from openai");
    } else {
      ...
      try {
        ...
        warning4(`release notes: error from github: ${e.message}`);
      }
    }
  }
  ...
  if (!options.disableReview) {
    ...
    const doReview = async (filename, fileContent, patches) => {
      ...
      for (const [startLine, endLine, patch] of patches) {
        if (context2.payload.pull_request == null) {
          warning4("No pull request found, skipping.");
          continue;
        }
        if (patchesPacked >= patchesToPack) {
          ...
          if (options.debug) {
            info4(`prompt so far: ${prompts.renderReviewFileDiff(ins)}`);
          }
          break;
        }
        patchesPacked += 1;
        ...
        try {
          ...
          if (allChains.length > 0) {
            info4(`Found comment chains: ${allChains} for ${filename}`);
...
`;
      ...
      if (patchesPacked > 0) {
        try {
          ...
          if (response === "") {
            info4("review: nothing obtained from openai");
...
${review.comment}`;
  ...
  for (const line of lines) {
    ...
    if (lineNumberRangeMatch != null) {
      ...
      currentComment = "";
      if (debug) {
        info4(`Found line number range: ${currentStartLine}-${currentEndLine}`);
      }
      continue;
    }
...
import { info as info5, warning as warning5 } from "@actions/core";
import { context as github_context2 } from "@actions/github";
var context3 = github_context2;
...
var handleReviewComment = async (heavyBot, options, prompts) => {
  ...
  if (!context3.payload) {
    warning5(`Skipped: ${context3.eventName} event is missing payload`);
    return;
  }
  const comment = context3.payload.comment;
  if (comment == null) {
    warning5(`Skipped: ${context3.eventName} event is missing comment`);
    return;
  }
  if (context3.payload.pull_request == null || context3.payload.repository == null) {
    warning5(`Skipped: ${context3.eventName} event is missing pull_request`);
    return;
  }
  ...
  if (context3.payload.action !== "created") {
    warning5(`Skipped: ${context3.eventName} event is not created`);
    return;
  }
  if (!comment.body.includes(COMMENT_TAG) && !comment.body.includes(COMMENT_REPLY_TAG)) {
    ...
    if (!topLevelComment) {
      warning5("Failed to find the top-level comment to reply to");
      return;
    }
    ...
    if (commentChain.includes(COMMENT_TAG) || commentChain.includes(COMMENT_REPLY_TAG) || comment.body.includes(ASK_BOT)) {
      ...
      try {
        ...
        warning5(`Failed to get file diff: ${error2}, skipping.`);
      }
    ...
    info5(`Skipped: ${context3.eventName} event is from the bot itself`);
  }
};
...
async function run() {
  const options = new Options(
    ...
    getInput3("system_message"),
    getInput3("openai_light_model"),
    getInput3("openai_heavy_model"),
    getInput3("openai_model_temperature"),
    getInput3("openai_retries"),
    getInput3("openai_timeout_ms"),
    getInput3("openai_concurrency_limit"),
    getInput3("github_concurrency_limit"),
    getInput3("language")
  );
  options.print();
  const prompts = new Prompts(
    getInput3("summarize"),
    getInput3("summarize_release_notes")
  );
  let lightBot = null;
  try {
    lightBot = new Bot(
  ...
  try {
    if (process.env.GITHUB_EVENT_NAME === "pull_request" || process.env.GITHUB_EVENT_NAME === "pull_request_target") {
      ...
      warning6("Skipped: this action only works on push events or pull_request");
    }
  } catch (e) {
    if (e instanceof Error) {
      setFailed2(`Failed to run: ${e.message}, backtrace: ${e.stack}`);
    } else {
      setFailed2(`Failed to run: ${e}, backtrace: ${e.stack}`);
    }
  }
}
process.on("unhandledRejection", (reason, p) => {
  warning6(`Unhandled Rejection at Promise: ${reason}, promise is ${p}`);
}).on("uncaughtException", (e) => {
  warning6(`Uncaught Exception thrown: ${e}, backtrace: ${e.stack}`);
});
async function main() {
  await run();
}
